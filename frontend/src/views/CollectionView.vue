...
const currentSortIcon = computed(() => sortIcons[filters.value.sort] || '💰')

// Lógica Unificada para Mouse e Toque
...
async function loadCollection(reset = false) {
  ...
  try {
    const res = await fetch(`http://localhost:3000/collection?${params.toString()}`, {
...
async function selectVariant(variant) {
  ...
  if (selectedGroup.value) selectedGroup.value.cover_image = variant.image
  try {
    await fetch('http://localhost:3000/set-preference', {
      method: 'POST',
      headers: auth.authHeader(),
      body: JSON.stringify({ cardName: selectedGroup.value.name, cardId: variant.id })
    })
  } catch (e) { console.error(e) }
}
...
async function handleSell(variant) {
  ...
  try {
    const res = await store.sellCard({ id: variant.id, rarity: variant.rarity }, variant.is_foil)
...
async function handleSurplus() {
  ...
  const data = await store.sellSurplus()
  if (data.soldCount > 0) { auth.user.gold += data.goldEarned; loadCollection(true) }
}
...
async function handleSellAll() {
  ...
  try {
    loading.value = true
    const data = await store.sellAll()
    alert(`Venda concluída!\n\nCartas vendidas: ${data.soldCount}\nLucro: ${fmt(data.goldEarned)} moedas`)
    loadCollection(true)
  } catch (e) { alert('Erro ao vender: ' + e.message) } finally { loading.value = false }
}
  ...
  <div class="relative min-h-full pb-20">
    ...
    <header class="sticky top-0 z-20 bg-zinc-950/95 backdrop-blur border-b border-zinc-800 px-4 py-3 shadow-lg">
      ...
      <div class="flex items-center gap-2 px-1">
        ...
        <div class="relative shrink-0 w-10 h-10">
          <div
            class="absolute inset-0 bg-zinc-900 rounded-full flex items-center justify-center border border-zinc-700 shadow-sm pointer-events-none text-lg"
          >
            {{ currentSortIcon }}
          </div>
          ...
          <select
            ...
            class="absolute inset-0 opacity-0 w-full h-full cursor-pointer appearance-none bg-transparent"
        ...
        <div class="w-px h-6 bg-zinc-800 shrink-0 mx-1"></div>
        ...
        <div class="flex items-center gap-2 flex-1 justify-between">
          <button
            ...
            class="w-9 h-9 rounded-full flex items-center justify-center transition border border-transparent relative shrink-0 active:scale-90"
            :class="filters.colors.includes(c.code) ? `bg-zinc-800 scale-110 shadow-lg ring-1 ${c.ring}` : 'bg-zinc-900/50 opacity-60 grayscale hover:opacity-100 hover:bg-zinc-800'"
          >
            <img
              :src="c.icon"
              class="w-4 h-4 invert transition-opacity"
    ...
    <div class="p-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
      ...
      >
        ...
        >x{{ group.total_quantity }}</div>
        <div
          class="absolute bottom-0 w-full h-1.5"
          :class="{ 'bg-zinc-600': group.main_rarity === 'common', 'bg-blue-400': group.main_rarity === 'uncommon', 'bg-yellow-400': group.main_rarity === 'rare', 'bg-orange-600 shadow-[0_0_12px_rgba(234,88,12,0.8)]': group.main_rarity === 'mythic' }"
        ></div>
      </div>
    </div>
    ...
    ><button
      ...
      >{{ loading ? '...' : 'Carregar Mais' }}</button></div>
        ...
        >
          ...
          <div class="flex-1 overflow-y-auto p-4 bg-zinc-950 space-y-3 pb-safe">
            <div
              ...
              :class="previewVariant.id === variant.id && previewVariant.is_foil === variant.is_foil ? 'bg-zinc-800/80 border-indigo-500/50 shadow-lg ring-1 ring-indigo-500/20' : 'bg-zinc-900/40 border-zinc-800 hover:bg-zinc-900'"
            >
              <div
              ...
              <div class="flex-1 min-w-0 py-1">
                <div class="flex items-center gap-2 mb-1"><span
                  ...
                  >{{ variant.set_code }}</span></div>
                <div class="text-sm text-zinc-300"><span class="text-white font-bold text-base">{{ variant.quantity
                    }}</span> cópias</div>
              </div>
              <div
              ...
              >
                ...
                >
                  ...
                  >🪙</span><span>{{ fmt(variant.sell_price) }}</span>
                </button>
              </div>
            </div>
            <div class="h-6"></div>
...
<style scoped>
/* Força o dropdown a ser escuro em navegadores Web */
select option {
