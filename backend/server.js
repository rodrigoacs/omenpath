...
// --- CONFIGURAÇÃO ---
const pool = new Pool({
  user: process.env.DB_USER || 'acs',
  host: process.env.DB_HOST || 'librishelf.com',
  database: process.env.DB_NAME || 'omenpath',
  password: process.env.DB_PASS || 'acs1405',
...
const FOIL_CHANCE = 0.1      // 10% de chance de Foil
const FOIL_MULTIPLIER = 2    // Foils valem 2x mais
...
app.get('/', (req, res) => res.json({ status: 'Omenpath Online 💎' }))
...
// === COLLECTION VIEW (COM FOIL E PERSISTÊNCIA) ===
  ...
  try {
    ...
    const sqlBasePrice = `
      COALESCE(
        ...
        NULLIF(c.price_eur, 0) * ${EUR_TO_USD},
        NULLIF(c.price_tix, 0) * ${TIX_TO_USD},
        CASE c.rarity WHEN 'mythic' THEN 1.00 WHEN 'rare' THEN 0.25 WHEN 'uncommon' THEN 0.10 ELSE 0.02 END
      )
    `
    // Preço Venda Base (Sem multiplicador foil ainda)
    const sqlSellPrice = `FLOOR(${sqlBasePrice} * ${EXCHANGE_RATE})`
    ...
    const query = `
      SELECT 
        ...
        -- CAPA PERSISTENTE: Usa a preferência se houver
        COALESCE(
        ...
        JSON_AGG(json_build_object(
           ...
           -- PREÇO COM MULTIPLICADOR DE FOIL (2x)
           'sell_price', (
              CASE 
...
app.get('/booster/:setCode', async (req, res) => {
  ...
  try {
    ...
    // Agregação para Insert Otimizado
    const aggMap = new Map()
    for (const card of allCards) {
      // Chave única: ID + FoilStatus
    ...
    res.json({ success: true, new_balance: userRes.rows[0].gold - totalCost, cards: allCards, opened: count })
...
app.post('/sell', async (req, res) => {
  ...
  try {
    ...
    let sellPrice = Math.max(1, Math.floor(realPriceUsd * EXCHANGE_RATE))

    // Multiplicador Foil na Venda
...
app.get('/oracle', async (req, res) => {
  const { q, userId } = req.query // q = termo de busca

  if (!q || q.length < 3) {
    return res.json([]) // Só busca com 3+ caracteres para não travar o banco
  }
  ...
  try {
    const query = `
      SELECT 
        ...
        c.image_uri,
        s.name as set_name,
        COALESCE(SUM(col.quantity), 0)::int as owned_count,
        MAX(col.is_foil::int) as has_foil -- 1 se tiver pelo menos uma foil
      FROM cards c
      JOIN sets s ON c.set_code = s.code
      LEFT JOIN collection col ON c.id = col.card_id AND col.user_id = $1
      WHERE c.name ILIKE $2 AND c.is_booster = true
      GROUP BY c.id, s.name, s.released_at
      ORDER BY 
        (c.name ILIKE $2) DESC, -- Exatos primeiro
        c.name ASC, 
        s.released_at DESC
      LIMIT 50
    `
...
// MIDDLEWARE DE PROTEÇÃO (Para usar no futuro nas rotas sensíveis)
...
app.listen(3000, () => console.log('🔥 Server Omenpath 3000 (Foil & Persistence Active)'))